...
from doctorio.rest.serializers.doctors import (
    ...
    MedicineSerializer,
    PrivateDoctorUpdateNameSerializer,
...
class PrivateMeDoctorDetail(RetrieveAPIView):
...
class PrivateDoctorUpdateName(UpdateAPIView):
    serializer_class = PrivateDoctorUpdateNameSerializer
    permission_classes = [IsDoctor]
...
class PrivateDoctorMedicineDetail(RetrieveAPIView):
    ...
    def get_object(self):
        return get_object_or_404(
            self.queryset.filter(), uid=self.kwargs.get("uid", None)
        )
